function [Gu, Gm] = LS_forward(H, Vu, Vm, M1, n0, Bu, Bm, upower, mpower,mmmm)
...
for user_idx = 1 : M
    for sym_idx = 1:M1
        Y(:,sym_idx,user_idx) = H(:,:,user_idx,user_idx)*(upower(user_idx)*Vu(:,user_idx)*Bu(sym_idx,user_idx) + mpower(user_idx)*Vm(:,user_idx)*Bm(sym_idx,user_idx)) + mmmm(:,sym_idx);
